...
@section('content')
    <div class="container w-full bg-white p-3 rounded-xl shadow-xl">
        <div class="row m-3">
            {!! Form::open(['method'=>'POST','route'=>['typeljoists.store']]) !!}
            <input type="hidden" name="Quotation_Id" value="{{$Quotation_Id}}">
            <div  class="row bg-white p-4 shadow-lg rounded-lg">
                <div class="col-sm-6 col-xs-12">
                    <h2><i class="fa-solid fa-crop-simple"></i>&nbsp;Vigas Tipo L 2.5</h2>
                    ...
                    <div class="form-group p-2">
                        ...
                        <select name="joist_type" class="inputjet w-full text-xs uppercase">
                            @foreach ($Joists as $row)
                                <option value="{{$row->joist}}"@if (old('joist')==$row->joist) selected @endif>{{$row->joist}}</option>
                            @endforeach
                        </select>
